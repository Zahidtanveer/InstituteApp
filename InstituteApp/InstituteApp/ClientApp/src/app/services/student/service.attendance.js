...
var StudentAttendanceService = /** @class */ (function () {
    function StudentAttendanceService(_http, baseUrl) {
        this._http = _http;
        this.myAppUrl = "";
        this.myAppUrl = baseUrl;
    }
    StudentAttendanceService.prototype.getStudentAttendance = function (batch, course, date) {
        return this._http.get(this.myAppUrl + 'api/StudentAttendance/Index' + "?batch=" + batch + "&&course=" + course + "&&date=" + date)
            .map(function (response) { return response.json(); })
            .catch(this.errorHandler);
    };
    StudentAttendanceService.prototype.getStudentAttendanceById = function (id) {
    ...
    StudentAttendanceService.prototype.updateStudentAttendance = function (studentAttendance) {
        return this._http.put(this.myAppUrl + 'api/StudentAttendance/Edit', studentAttendance)
            .map(function (response) { return response.json(); })
            .catch(this.errorHandler);
    };
    StudentAttendanceService.prototype.deleteStudentAttendance = function (id) {
        return this._http.delete(this.myAppUrl + "api/StudentAttendance/Delete/" + id)
            .map(function (response) { return response.json(); })
            .catch(this.errorHandler);
    };
    StudentAttendanceService.prototype.errorHandler = function (error) {
        console.log(error);
        return Observable_1.Observable.throw(error);
    };
    StudentAttendanceService = __decorate([
        core_1.Injectable(),
